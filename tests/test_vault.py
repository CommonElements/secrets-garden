...
class TestVaultManager:
    ...
    def test_create_vault(self, vault_manager: VaultManager):
        ...
        with open(vault_manager.config_path) as f:
            config = json.load(f)
    ...
    def test_password_verification_security(self, vault_manager: VaultManager):
        ...
        password = "SecureP@ssw0rd!ForTesting"
        vault_manager.create(password, "Security test vault", validate_password=False)

        # Should not be able to unlock with similar passwords
        similar_passwords = [
            "SecureP@ssw0rd!ForTestin",  # Missing letter
            "SecureP@ssw0rd!ForTestinG", # Wrong case
            "SecureP@ssw0rd!ForTesting ", # Extra space
            "secureP@ssw0rd!ForTesting",  # Different case
            "",                           # Empty
        ]

        for bad_password in similar_passwords:
            # With brute force protection, we expect either InvalidPasswordError or VaultError
            with pytest.raises((InvalidPasswordError, VaultError)):
